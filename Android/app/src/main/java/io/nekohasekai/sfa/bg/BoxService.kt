...
class BoxService(private val service: Service, private val platformInterface: PlatformInterface) : CommandServerHandler {
    ...
    private suspend fun startService() {
        try {
            ...
            val storedContent = profileFile.readText()
    ...
    private fun normalizeStoredProfile(
        profileFile: File,
        storedContent: String,
    ): String {
        val normalized = ClashConfigNormalizer.normalize(storedContent).content
        ProfileConfigStore.writeIfChanged(profileFile, normalized)
        return normalized
    }

    private fun applyConfiguredNodeSelections(content: String) {
        val selections =
            runCatching {
                ProfileNodeSelection.read(content).filter { it.hasExplicitSelection }
            }.getOrElse {
                Log.w(TAG, "Failed to read configured node selections", it)
                return
            }
        ...
        selections.forEach { selection ->
            runCatching {
                client.selectOutbound(selection.tag, selection.selected)
            }.onFailure {
                Log.w(TAG, "Failed to apply a configured node selection", it)
            }
        }
    }
    ...
    internal fun sendNotification(notification: Notification) {
        val builder =
            NotificationCompat.Builder(service, notification.identifier).setShowWhen(false)
                ...
                .setOnlyAlertOnce(true).setSmallIcon(R.drawable.ic_clashnl_menu)
