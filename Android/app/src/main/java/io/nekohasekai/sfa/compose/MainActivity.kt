...
import io.nekohasekai.sfa.compose.component.CurvedBottomNavigationBar
...
class MainActivity :
    ...
    private var showPrivacyDisclosure by mutableStateOf(false)
    private val privacyPreferences by lazy {
        getSharedPreferences("clashnl_privacy", MODE_PRIVATE)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        showPrivacyDisclosure = !privacyPreferences.getBoolean("vpn_disclosure_v1", false)
    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (
            (uri.scheme == "clashnl" && uri.host == "import-remote-profile") ||
            ((uri.scheme == "clash" || uri.scheme == "clashmeta") && uri.host == "install-config")
        ) {
            val remoteURL = uri.getQueryParameter("url")
            if (remoteURL == null || (!remoteURL.startsWith("http://") && !remoteURL.startsWith("https://"))) {
                pendingIntentErrorMessage = getString(R.string.error_profile_link_url)
                return
            }
            ...
            val name =
                uri.getQueryParameter("name")
                    ...
                    ?: remoteUri.lastPathSegment
                        ?.substringBeforeLast(".")
                        ?.takeIf { it.isNotBlank() }
                    ?: remoteUri.host
                    ?: getString(R.string.imported_profile_default_name)
            pendingImportProfile = Triple(name, remoteUri.host ?: remoteURL, remoteURL)
    ...
    @SuppressLint("NewApi")
    fun startService() {
        if (!privacyPreferences.getBoolean("vpn_disclosure_v1", false)) {
        ...
        if (Settings.dynamicNotification &&
            ...
            !ServiceNotification.checkPermission()
        ) {
    ...
    private fun startService0() {
        if (::dashboardViewModel.isInitialized) {
            dashboardViewModel.updateServiceStatus(Status.Starting)
        }
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                if (prepare()) {
                ...
                withContext(Dispatchers.Main) {
                    onServiceAlert(Alert.StartService, exception.message)
                    cancelPendingDashboardStart()
                }
            }
        }
    ...
    private fun cancelPendingDashboardStart() {
        if (::dashboardViewModel.isInitialized) {
            dashboardViewModel.cancelPendingServiceStart()
        }
    }
    ...
    fun SFAApp() {
                        ...
                        if (result == androidx.compose.material3.SnackbarResult.ActionPerformed) {
                            try {
                                when (modeToShow) {
                                    UiEvent.ApplyServiceChange.Mode.Reload -> {
                                        withContext(Dispatchers.IO) {
                                            ProfileRuntime.reloadSelectedIfRunning(this@MainActivity)
                                        }
                                    }
        ...
        var showUpdateCheckPrompt by remember {
            mutableStateOf(Vendor.hasCustomUpdate && !Settings.updateCheckPrompted)
        }
        ...
        val currentRootRoute =
            when {
                ...
                isProfileRoute -> Screen.Subscriptions.route
                else -> currentRoute
            }
        val isConnectionsRoute = currentRootRoute == Screen.Connections.route
        val isSubscriptionsRoute = currentRootRoute == Screen.Subscriptions.route
        ...
        val allowedRoutes =
            buildSet {
                ...
                // Logs remain available to internal diagnostics while the
                // old libbox-backed tools are not part of the Mihomo app.
        ...
        val scaffoldContent: @Composable (PaddingValues) -> Unit = { paddingValues ->
            Box(
                ...
                // Local VPN control lives exclusively on the dashboard service
                // card. Other root pages stay focused on their own task.
                ...
                SFANavHost(
                    ...
                    onOpenNodePicker = { showGroupsSheet = true },
                    ...
                    bottomBar = {
                        if (!isSubScreen) {
                            ...
                            CurvedBottomNavigationBar(
                                ...
                                selectedRoute = currentRootRoute,
                            ...
                            Row(
                                ...
                                when {
                                    currentServiceStatus == Status.Stopped -> {
                                        TextButton(
                                            onClick = dashboardViewModel::toggleService,
                                            enabled = dashboardUiState.selectedProfileId >= 0,
                                        ) {
                                            Text(stringResource(R.string.dashboard_start_service))
                                        }
                                    }
                                    ...
                                    groupsUiState.groups.isNotEmpty() -> {
            ...
            AlertDialog(
                    ...
                    TextButton(
                        onClick = {
                            privacyPreferences.edit()
                                .putBoolean("vpn_disclosure_v1", true)
                                .apply()
                ...
                dismissButton = {
                    TextButton(onClick = { finish() }) {
                        Text(stringResource(R.string.vpn_disclosure_leave))
                    }
                },
            )
        }
