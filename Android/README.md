...
`sing-box-for-android` 的真实 `VpnService`/TUN 实现，运行时使用固定版本
sing-box Libbox；导入 Clash/Mihomo YAML 时，先在本地转换并校验为原生
sing-box JSON，再保存和启动。
...
- Clash/Mihomo YAML 与原生 sing-box JSON 自动识别
- 远程订阅首次拉取、手动更新和后台更新时自动转换
- 解析订阅响应头及正文注释中的流量、总量、到期时间与建议刷新间隔，
  并在配置卡片展示最新订阅状态
- 独立“订阅”导航和居中的可配置仪表盘；九类模块均可开关和拖动排序
- 应用内语言选择，以及 HTTP(S) URL/普通订阅二维码直接导入
- Shadowsocks、VMess、VLESS Reality、Trojan、Hysteria 1/2、TUIC v5、
  AnyTLS、ShadowTLS、Naive、SSH、WireGuard endpoint 等共享转换能力
- 首次启动 VPN 流量披露；未确认前不会发起 VPN 权限请求
- 独立应用 ID `com.clashnl.android` 和 ClashNl 品牌资源

转换后的运行配置保存在应用私有目录中。转换器对没有可靠等价语义的项目
明确返回错误，避免静默丢规则。
...
先构建包含 ClashNl 转换 API 的 Libbox AAR：

```bash
cd /path/to/iosClashNL/Android
