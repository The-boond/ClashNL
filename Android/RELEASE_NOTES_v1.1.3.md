...
- Wi-Fi 物理出口为 `112.93.145.105`，蜂窝物理出口为 `14.24.244.144`；两种入口下当前日本节点出口均为 `64.204.48.61`。
- USB 共享客户端在 VPN 开关前后均显示 `112.93.145.105`，证明 Android 普通 `VpnService` 不会透明接管热点/USB 转发流量。本版本没有宣称支持免 Root 的透明 VPN 热点。
