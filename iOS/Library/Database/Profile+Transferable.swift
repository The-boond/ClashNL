...
public extension LibboxProfileContent {
    ...
    func importProfile() async throws -> Profile {
        ...
        let rawType = type
        guard let profileType = ProfileType(rawValue: Int(rawType)) else {
            throw NSError(
                domain: "ProfileImport",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: String(localized: "Unknown profile type")]
            )
        }
        let rawConfig = config
        var normalizedRemotePath = remotePath.trimmingCharacters(in: .whitespacesAndNewlines)
        let autoUpdate = autoUpdate
        let autoUpdateInterval = autoUpdateInterval
        let lastUpdated = lastUpdated
        var subscriptionMetadata: SubscriptionMetadata?
        var importedConfig = rawConfig

        if profileType == .remote {
            guard let normalizedURL = RemoteProfileURLPolicy.normalize(normalizedRemotePath) else {
                throw NSError(
            ...
            normalizedRemotePath = normalizedURL
            if importedConfig.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                let response = try await HTTPClient.getSubscriptionAsync(normalizedURL)
                importedConfig = response.content
                subscriptionMetadata = SubscriptionMetadataParser.parse(
        ...
        let config = try await BlockingIO.run {
            try ProfileContentNormalizer.normalize(importedConfig)
        }
        ...
        let profileName = requestedName.isEmpty && profileType == .remote
            ? RemoteProfileURLPolicy.suggestedName(for: normalizedRemotePath)
            : requestedName
        ...
        let profile = Profile(
            ...
            remoteURL: profileType == .remote ? normalizedRemotePath : nil,
