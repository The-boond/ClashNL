...
enum Database {
    ...
    private static func makeShared() -> any DatabaseWriter {
        do {
            ...
            migrator.registerMigration("add_subscription_metadata") { db in
                try db.alter(table: "profiles") { t in
                    t.add(column: "subscriptionUpload", .integer)
                    t.add(column: "subscriptionDownload", .integer)
                    t.add(column: "subscriptionTotal", .integer)
                    t.add(column: "subscriptionExpireAt", .datetime)
                    t.add(column: "subscriptionUpdateInterval", .integer).notNull().defaults(to: 0)
                    t.add(column: "subscriptionWebPageURL", .text)
                }
            }
