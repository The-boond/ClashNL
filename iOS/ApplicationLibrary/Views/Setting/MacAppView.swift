...
public struct AppView: View {
        ...
        Group {
            if isLoading {
                ...
                FormView {
                        ...
                        .onChangeCompat(of: appearanceMode) { newValue in
                            Task {
                                await SharedPreferences.appAppearanceMode.set(newValue.rawValue)
                                NotificationCenter.default.post(name: .appAppearanceModeDidChange, object: nil)
                            }
                        }
                    #endif
    ...
    private func loadSettings() async {
        selectedLanguage = Self.currentLanguage()
            ...
            appearanceMode = AppAppearanceMode(
                storedValue: await SharedPreferences.appAppearanceMode.get()
            )
        #endif
        #if os(macOS)
    ...
    private static func configuredLanguageCodes() -> [String] {
           ...
           !configured.isEmpty
        {
            ...
            // Keep the picker useful in development and on targets whose
            // generated Info.plist omits CFBundleLocalizations.
            rawCodes = fallbackLanguageCodes
        }
