...
public struct NewProfileMenuView: View {
    ...
    @State private var localImportRequest: NewProfileView.LocalImportRequest?
        ...
        @State private var showRemoteProfile = false
    #endif
    ...
    public var body: some View {
        ...
        private var macOSBody: some View {
            ...
            .fileImporter(
                ...
                allowedContentTypes: [.profile, .json, .plainText],
            ...
            .sheet(isPresented: $showRemoteProfile) {
                NewProfileView(initialProfileType: .remote, onSuccess: { profile in
                    await SharedPreferences.selectedProfileID.set(profile.mustID)
                    dismiss()
                })
                .environmentObject(environments)
    ...
    private var menuContent: some View {
        FormView {
            Section {
                ...
                #if os(macOS)
                    FormButton {
                        showRemoteProfile = true
                    } label: {
                        Label("Add Subscription URL", systemImage: "link")
                    }
                #else
                    FormNavigationLink {
                        NewProfileView(
                            initialProfileType: .remote,
        ...
        private func handleFileImport(_ result: Result<[URL], Error>) {
            Task { @MainActor in
                do {
                    ...
                    if ["json", "json5", "yaml", "yml", "conf", "txt"].contains(url.pathExtension.lowercased()) {
        ...
        private func handleQRCodeString(_ string: String) {
            if let request = remoteImportRequest(from: string) {
            ...
            guard let remoteProfile,
                  let normalizedURL = RemoteProfileURLPolicy.normalize(remoteProfile.url)
            else {
                alert = AlertState(errorMessage: String(localized: "The QR code does not contain a valid profile import link."))
                return
            }
            let requestedName = remoteProfile.name.trimmingCharacters(in: .whitespacesAndNewlines)
            importRequest = NewProfileView.ImportRequest(
                name: requestedName.isEmpty
                    ? RemoteProfileURLPolicy.suggestedName(for: normalizedURL)
                    : requestedName,
                url: normalizedURL
            )
        }

        private func remoteImportRequest(from value: String) -> NewProfileView.ImportRequest? {
            let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
            if let normalizedURL = RemoteProfileURLPolicy.normalize(trimmed) {
                return NewProfileView.ImportRequest(
                    name: RemoteProfileURLPolicy.suggestedName(for: normalizedURL),
                    url: normalizedURL
                )
            }
            ...
            guard let components = URLComponents(string: trimmed),
                  ...
                  (["clash", "clashmeta"].contains(scheme) && host == "install-config") ||
                  (["clashnl", "sing-box"].contains(scheme) && host == "import-remote-profile"),
                  let rawURL = components.queryItems?.first(where: { $0.name.lowercased() == "url" })?.value,
                  let normalizedURL = RemoteProfileURLPolicy.normalize(rawURL)
            else {
                return nil
            }
            let requestedName = (
            ...
            let profileName = requestedName.flatMap { $0.isEmpty ? nil : $0 }
                ?? RemoteProfileURLPolicy.suggestedName(for: normalizedURL)
            return NewProfileView.ImportRequest(
                name: profileName,
                url: normalizedURL
