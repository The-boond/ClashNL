...
struct MainView: View {
    ...
    var body: some View {
        mainBody
            .preferredColorScheme(Variant.screenshotMode ? .dark : appearanceMode.preferredColorScheme)
            .task {
                await reloadAppearanceMode()
            }
            .onReceive(NotificationCenter.default.publisher(for: .appAppearanceModeDidChange)) { _ in
                Task {
                    await reloadAppearanceMode()
                }
            }
    ...
    private func reloadAppearanceMode() async {
        let storedValue = await SharedPreferences.appAppearanceMode.get()
        appearanceMode = AppAppearanceMode(storedValue: storedValue)
    }

    private struct AccessoryInset<StatusBar: View, FAB: View>: View {
        @ObservedObject var profile: ExtensionProfile
        let showDisconnectedFAB: Bool
        @ViewBuilder let statusBar: () -> StatusBar
        @ViewBuilder let fab: () -> FAB

        var body: some View {
            ZStack(alignment: .bottomTrailing) {
                if profile.status == .disconnected, showDisconnectedFAB {
                    fab()
